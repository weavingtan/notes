...
- [x] 在 `scripts/build.mjs` 的 `SITE_STYLES` 与 `buildHtmlSkeleton` 中，为全站 `:root` 及各个主题选择器（`[data-theme-style="..."]`）定义 `--hero-overlay-gradient`：
...
- [x] 将以下静态渐变替换为 `var(--hero-overlay-gradient)`：
...
- [x] 在 `scripts/test-site.mjs` 中添加针对 `--hero-overlay-gradient` 和 5 套主题选择器的自动化断言。

---

### Task 2: 关于页右侧留白空间策展（技能装备矩阵 & Now 状态看板）
**涉及文件**: `scripts/build.mjs`, `scripts/test-site.mjs`
- [x] 在 `buildAboutHtml` 的右侧列 `.about-interests-col` 下方注入：
  - `.about-craft-matrix`（架构与工程技术栈 + 设计与生产力工具药丸微标组）；
  - `.about-now-card`（当前实时打磨焦点 + 呼吸绿点动画 + 状态说明）；
- [x] 在 `SITE_STYLES` 中编写对应的卡片样式，包含 `var(--bg-card)`、`var(--border-color)`、`var(--primary)` 动态主题绑定，以及微上浮交互动画；
- [x] 严格遵循 AGENTS.md 约束：全部使用 `<section style="box-sizing: border-box; ...">`，绝无 `<div>` 与 `grid`；
- [x] 在 `scripts/test-site.mjs` 中添加针对 `about-craft-matrix` 与 `about-now-card` 节点的结构断言。

---

### Task 3: 关于页正文 100% 等宽与现代杂志双栏流式重构
**涉及文件**: `scripts/build.mjs`, `scripts/test-site.mjs`
- [x] 彻底移除 `.about-post-body` 中的 `max-width: min(94vw, 820px)`，重构为 `width: 100%; max-width: 100%; margin: 56px 0 72px 0;`，与顶部 1280px 完全齐平；
- [x] 打造现代杂志双栏章节版式（Editorial Split Chapter）：
...
- [x] 运行构建并校验 `dist/about.html`，确认上下宽度视觉统一、层次饱满。

---

### Task 4: 全套自动化测试套件扩展、微信 0 div 验证与全站构建部署
**涉及文件**: `scripts/test-site.mjs`, `dist/*.html`
- [x] 运行 `node scripts/build.mjs && node scripts/test-site.mjs`；
- [x] 自动化扫描 `dist/` 下全部 12 个 HTML 页面，验证 `<main>` 内部 **`<div>` 标签为 0**；
- [x] 验证全站 18+ 测试套件 100% 通过；
- [x] 提交修改并推送至 GitHub `main` 分支。
